...
# @@File             :  install.sh
# @@Description      :  Install configurations for git
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Setup plugins
PLUGIN_REPOS="https://github.com/arialdomartini/oh-my-git"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Grab release from github releases
LATEST_RELEASE=""
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Specify global packages
GLOBAL_OS_PACKAGES="git "
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__run_post_install() {
  local getRunStatus=0
  __mkdir "$HOME/.config/git"
  __mkdir "$HOME/.config/local"
  __mkdir "$HOME/.config/secure/git"
  [ -f "$APPDIR/gitmessage" ] && [ ! -f "$HOME/.config/secure/git/message" ] && __cp_rf "$APPDIR/gitmessage" "$HOME/.config/secure/git/message" || true
  [ -f "$APPDIR/gitignore" ] && [ ! -f "$HOME/.config/secure/git/ignore" ] && __cp_rf "$APPDIR/gitignore" "$HOME/.config/secure/git/ignore" || true
  [ -f "$APPDIR/gitattributes" ] && [ ! -f "$HOME/.config/secure/git/attributes" ] && __cp_rf "$APPDIR/gitattributes" "$HOME/.config/secure/git/attributes" || true
  [ -f "$APPDIR/git-credentials" ] && [ ! -f "$HOME/.config/secure/git/credentials" ] && __mv_f "$APPDIR/git-credentials" "$HOME/.config/secure/git/credentials" || true
  [ -f "$HOME/.git-credentials" ] && [ ! -f "$HOME/.config/secure/git/credentials" ] && __mv_f "$HOME/.git-credentials" "$HOME/.config/secure/git/credentials" || true
  [ -L "$HOME/.git-credentials" ] && __rm_link "$HOME/.git-credentials" || true
  __symlink "$APPDIR/gitconfig" "$HOME/.gitconfig"
  return $getRunStatus
}
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
